...
import {Camera, IdCard, KeyRound, LoaderCircle, LockKeyhole, ShieldCheck, TriangleAlert} from '@lucide/vue';
...
const errorText = ref('');
const statusText = ref('Camera audit is required to unlock the attendance system.');
...
const submitUnlock = async () => {
    ...
    if (!selectedCredential.value) {
        errorText.value = method.value === 'rfid' ? 'Scan an RFID card first.' : 'Enter your unlock password first.';
        return;
    }

    const auditImage = captureAuditImage();
    if (!auditImage) {
        errorText.value = 'Camera is not ready. Allow camera access before unlocking.';
        return;
    }
    ...
    try {
        ...
        statusText.value = 'Camera audit is required to unlock the attendance system.';
        rfidBuffer.value = '';
...
onMounted(async () => {
    try {
        ...
        errorText.value = error instanceof Error ? error.message : 'Failed to start.';
    }
});
...
<template>
    ...
    <main class="min-h-screen bg-brand-bg px-4 py-6 text-brand-stroke md:px-8">
        <section class="mx-auto grid min-h-[calc(100vh-3rem)] max-w-xl items-center">
            <video
                ref="videoRef"
                autoplay
                playsinline
            ...
            <div
                ...
                <div class="mb-8 space-y-2">
                    ...
                    <h1 class="text-3xl font-black leading-tight md:text-4xl">Unlock attendance station</h1>
                    <p class="text-sm font-semibold text-brand-bg">{{ statusText }}</p>
                </div>
                ...
                <div class="mb-5 grid grid-cols-2 gap-3">
                    ...
                    <button
                        type="button"
                        class="inline-flex items-center justify-center gap-2 rounded-2xl border-2 border-brand-stroke px-4 py-3 text-sm font-black transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[4px_4px_0px_0px_#001e1d]"
                        :class="method === 'rfid' ? 'bg-brand-accent' : 'bg-white'"
                ...
                <div v-else class="space-y-4">
                    ...
                    <div class="rounded-2xl border-2 border-dashed border-brand-stroke bg-white/60 p-5 text-center">
                        <IdCard class="mx-auto mb-2 h-8 w-8"/>
                        <p class="text-sm font-black uppercase tracking-widest">Waiting for RFID scan</p>
                    </div>
                </div>
