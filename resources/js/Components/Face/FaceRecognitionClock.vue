    ...
    for (const employee of employeesWithPhotos.value) {
        try {
            ...
            labeledDescriptors.push(
                new faceapi.LabeledFaceDescriptors(employee.employee_id, [detection.descriptor]),
            );
        } catch (error) {
...
const recognizeFace = async () => {
    if (!videoRef.value || !overlayRef.value || !faceMatcher || isSubmitting.value) return;
    ...
    resizedDetections.forEach((detection, index) => {
        ...
        const employee = props.employees.find((item) => item.employee_id === result?.label) ?? null;
...
const startRecognitionLoop = () => {
    ...
    recognitionInterval = setInterval(() => {
        recognizeFace().catch((error) => {
            ...
            statusText.value = 'Recognition failed. Check profile photos and lighting.';
        });
    }, 900);
};
...
onMounted(async () => {
    ...
    try {
        ...
        console.error(error);
        statusText.value = error instanceof Error ? error.message : 'Unable to start facial recognition.';
        toast.add({
            severity: 'error',
...
<template>
    <section class="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <aside class="flex flex-col gap-6">
            <div class="rounded-3xl border-2 border-brand-stroke bg-brand-card p-6 shadow-[8px_8px_0px_0px_#001e1d]">
                <div class="mb-5 flex items-center gap-3">
                    <div class="rounded-xl border border-brand-stroke bg-brand-accent p-2">
                        <Clock class="h-5 w-5 text-brand-stroke"/>
                    </div>
                    <div>
                        <p class="text-xs font-black uppercase text-brand-bg">{{ currentDate }}</p>
                        <h1 class="text-3xl font-black tabular-nums text-brand-stroke">{{ currentTime }}</h1>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <button
                        ...
                        :class="attendanceType === 'time-in' ? 'bg-brand-accent' : 'bg-white'"
                    ...
                    <button
                        type="button"
                        class="flex items-center justify-center gap-2 rounded-2xl border-2 border-brand-stroke px-4 py-4 text-sm font-black text-brand-stroke shadow-[4px_4px_0px_0px_#001e1d] active:translate-x-1 active:translate-y-1 active:shadow-none"
                        :class="attendanceType === 'time-out' ? 'bg-brand-tertiary text-brand-headline' : 'bg-white'"
            ...
            <div class="rounded-3xl border-2 border-brand-stroke bg-brand-card p-6 text-brand-stroke shadow-[8px_8px_0px_0px_#001e1d]">
                <div class="mb-5 flex items-center gap-3">
                    <div class="rounded-xl border border-brand-stroke bg-brand-paragraph p-2">
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div class="rounded-2xl border border-brand-stroke/20 bg-white p-4">
                            ...
                            <p class="text-2xl font-black">{{ enrolledCount }}</p>
                            <p class="text-xs font-bold text-brand-bg">Enrolled</p>
                        </div>
                        <div class="rounded-2xl border border-brand-stroke/20 bg-white p-4">
                            <UserRound class="mb-2 h-5 w-5 text-brand-bg"/>
                            <p class="text-2xl font-black">{{ recognitionCount }}</p>
                            <p class="text-xs font-bold text-brand-bg">Detected</p>
                        </div>
                    </div>
                    ...
                    >
                        ...
                        <div class="min-w-0">
                            <div class="flex items-center gap-2">
                                ...
                                <p class="truncate font-black">{{ employeeName(matchedEmployee) }}</p>
                            </div>
                            <p class="truncate text-xs font-bold">{{ matchedEmployee.employee_id }} · {{ matchedEmployee.position }}</p>
                        </div>
                    </div>

                    <div v-if="isTraining" class="rounded-2xl border border-brand-stroke bg-brand-paragraph/40 p-4">
                        <p class="text-sm font-bold">Preparing {{ employeesWithPhotos.length }} profile photo{{ employeesWithPhotos.length === 1 ? '' : 's' }}...</p>
                    </div>
                </div>
            </div>
        </aside>
