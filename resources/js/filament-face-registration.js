...
window.faceRegistration = ({ registerUrl, existingFaces = [] }) => ({
    ...
    isBoxInsideOval(box, oval) {
        ...
        const normalizedCenter = (((centerX - oval.centerX) ** 2) / (oval.radiusX ** 2))
            + (((centerY - oval.centerY) ** 2) / (oval.radiusY ** 2));

        return normalizedCenter <= 0.82
    ...
    isPointInsideOval(point, oval, tolerance = 1) {
        return ((((point.x - oval.centerX) ** 2) / ((oval.radiusX * tolerance) ** 2))
            + (((point.y - oval.centerY) ** 2) / ((oval.radiusY * tolerance) ** 2))) <= 1;
    },
    ...
    areLandmarksInsideOval(landmarks, oval) {
        ...
        const insideCount = points.filter(point => this.isPointInsideOval(point, oval, 1.08)).length;
    ...
    sampleRegion(context, bounds, width, height) {
        ...
        const image = context.getImageData(xStart, yStart, xEnd - xStart, yEnd - yStart);
    ...
    analyzeFaceObstruction(detection, displaySize) {
        ...
        context.drawImage(this.video, 0, 0, displaySize.width, displaySize.height);

        const landmarks = detection.landmarks;
        const box = detection.detection.box;
        const faceStats = this.sampleRegion(context, box, displaySize.width, displaySize.height);
        const leftEyeStats = this.sampleRegion(
            context,
            this.pointBounds(landmarks.getLeftEye(), box.width * 0.08, box.height * 0.05),
            displaySize.width,
            displaySize.height,
        );
        const rightEyeStats = this.sampleRegion(
            context,
            this.pointBounds(landmarks.getRightEye(), box.width * 0.08, box.height * 0.05),
            displaySize.width,
            displaySize.height,
        );
        const noseStats = this.sampleRegion(
            context,
            this.pointBounds(landmarks.getNose(), box.width * 0.08, box.height * 0.04),
            displaySize.width,
            displaySize.height,
        );
        const mouthStats = this.sampleRegion(
            context,
            this.pointBounds(landmarks.getMouth(), box.width * 0.08, box.height * 0.07),
            displaySize.width,
            displaySize.height,
        );
        const eyeBandStats = this.sampleRegion(
            context,
            this.pointBounds([...landmarks.getLeftEye(), ...landmarks.getRightEye()], box.width * 0.12, box.height * 0.08),
            displaySize.width,
            displaySize.height,
        );
        ...
        const bridgeStats = this.sampleRegion(
            context,
            this.pointBounds([leftEye[0], leftEye[3], rightEye[0], rightEye[3]], box.width * 0.03, box.height * 0.06),
            displaySize.width,
            displaySize.height,
        );

        if (!faceStats || !leftEyeStats || !rightEyeStats || !noseStats || !mouthStats || !eyeBandStats || !bridgeStats) {
            return {blocked: true, message: 'Keep your full face visible.'};
        }

        const eyeMean = (leftEyeStats.mean + rightEyeStats.mean) / 2;
        const eyeDarkRatio = (leftEyeStats.darkRatio + rightEyeStats.darkRatio) / 2;
        const eyeLowDetail = leftEyeStats.stdDev < 13 && rightEyeStats.stdDev < 13;
        const eyesCovered = (eyeMean < faceStats.mean * 0.62 && eyeDarkRatio > 0.32)
            || (eyeDarkRatio > 0.52)
            || (eyeLowDetail && eyeDarkRatio > 0.28);

        if (eyesCovered) {
            return {blocked: true, message: 'Remove shades or anything covering the eyes.'};
        }

        const eyewearDetected = eyeBandStats.brightRatio > 0.08
        ...
        if (eyewearDetected) {
            return {blocked: true, message: 'Remove eyeglasses, shades, or anything covering the eyes.'};
        }

        const lowerFaceCovered = mouthStats.darkRatio > 0.46
            || mouthStats.brightRatio > 0.72
            || noseStats.darkRatio > 0.5
            || (mouthStats.stdDev < 10 && Math.abs(mouthStats.mean - faceStats.mean) > 35);

        if (lowerFaceCovered) {
            return {blocked: true, message: 'Remove any mask, hand, or object covering the face.'};
        }
    ...
    async scanFace() {
        ...
        this.faceInOval = detections.length === 1
        ...
        const obstruction = this.analyzeFaceObstruction(resizedDetections[0], displaySize);
    ...
    captureBlob() {
        ...
        const oval = this.ovalBounds(displaySize.width || this.video.videoWidth, displaySize.height || this.video.videoHeight);
        const scaleX = this.video.videoWidth / (displaySize.width || this.video.videoWidth);
        const scaleY = this.video.videoHeight / (displaySize.height || this.video.videoHeight);
        const sourceX = Math.max(0, oval.x * scaleX);
        const sourceY = Math.max(0, oval.y * scaleY);
        const sourceWidth = Math.min(this.video.videoWidth - sourceX, oval.width * scaleX);
        const sourceHeight = Math.min(this.video.videoHeight - sourceY, oval.height * scaleY);
    ...
    async save() {
        if (this.faceCount !== 1 || !this.faceInOval || !this.faceClear) {
            this.message = 'Center one clear, uncovered face inside the oval before saving.';
            this.success = false;
            return;
        }
